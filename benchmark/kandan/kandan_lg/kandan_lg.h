...
//ds[2]: [12] sorted-array : [table = Attachment, keys = (attachment:channel-channel:id,-attachment:created_at), cond = (((attachment:channel . channel:id) == Param (channel_id)) && ((attachment:created_at >= Value (0)) && (attachment:created_at <= Value (4294967295)))), value = memobj(Attachment-id,file_file_name,file_content_type,file_file_size,message_id,file_updated_at,created_at,updated_at,user_id,channel_id)]
//ds[3]: [16] Basic array: Channel, value = memobj(Channel-id,name,created_at,updated_at,user_id), nested = {
//  [28] sorted-array : [table = Channel::ActivitiesInChannel(16), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
//  [30] Basic array: Channel::ActivitiesInChannel(16), value = ptr to 1
//}
//
//ds[4]: [22] sorted-array : [table = Channel, keys = (-channel:id), cond = (channel:id == Param (channel_id)), value = memobj(Channel-id,name,created_at,updated_at,user_id), nested = {
//  [31] sorted-array : [table = Channel::ActivitiesInChannel(22), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
//}
//]
//ds[5]: [35] treeindex : [table = Activity, keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
//ds[6]: [43] treeindex : [table = Channel, keys = (-channel:name), cond = (channel:name == Param (name)), value = ptr to 16]
//
extern TreeIndex<oid_t, size_t, 10000000> idptr_ds_1;
...
extern BasicArray<Activity1, 10000000> activity_1;
...
extern SmallBasicArray<User4, 200000> user_4;
...
extern SortedArray<ds_12_key_type, Attachment12, 40000> ds_12;
...
public:
  ...
  oid_t channel_user_id;
    ...
    SortedArray<ds_31_key_type, ItemPointer, 20000> ds_31;
...
extern TreeIndex<ds_35_key_type, ItemPointer, 10000000> ds_35;
...
inline void init_ds_1_from_sql(MYSQL* conn, size_t oid) {
  ...
  if(oid%100000==0) printf("----ds 1 finish %u\n", oid/100000);
}
...
inline void init_ds_4_from_sql(MYSQL* conn, size_t oid) {
  char qs[2000];
  ...
  mysql_free_result(result);
  if(oid%2000==0) printf("----ds 4 finish %u\n", oid/2000);
}
...
//ds 35: [35] treeindex : [table = Activity, keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
inline void init_ds_35_from_sql(MYSQL* conn, size_t oid) {
  oid_t key_0 = 0;
  char qs[2000];
  sprintf(qs, "select activity.id as activity_id from activities as activity   where activity.id = %u and activity.id >= 1 and activity.id <= 10000000 group by activity_id order by activity_id", oid);
  std::string query_str(qs);
  ...
  mysql_free_result(result);
  if(oid%100000==0) printf("----ds 35 finish %u\n", oid/100000);
}
...
//ds 28: [28] sorted-array : [table = Channel::ActivitiesInChannel(16), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
...
//ds 31: [31] sorted-array : [table = Channel::ActivitiesInChannel(22), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (10000000))), value = ptr to 1]
