...
//}
//
//ds[1]: [4] Basic array: User, value = memobj(User-id,email,encrypted_password,reset_password_token,reset_password_sent_at,remember_created_at,first_name,last_name,signin_count,current_sign_in_at,current_sign_in_ip,last_sign_in_at,last_sign_in_ip,auth_token,locale,gravatar_hash,username,regstatus,active,is_admin,avatar_url,created_at,updated_at)
//ds[2]: [10] Basic array: Attachment, value = memobj(Attachment-id,user_id)
...
//  [28] sorted-array : [table = Channel::ActivitiesInChannel(16), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
//  [30] Basic array: Channel::ActivitiesInChannel(16), value = ptr to 1
//}
//
//ds[5]: [22] sorted-array : [table = Channel, keys = (-channel:id), cond = (channel:id == Param (channel_id)), value = memobj(Channel-id), nested = {
//  [31] sorted-array : [table = Channel::ActivitiesInChannel(22), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
//}
//]
//ds[6]: [27] treeindex : [table = Channel, keys = (-channel:id), cond = (channel:id == Param (channel_id)), value = ptr to 16]
//ds[7]: [35] treeindex : [table = Activity, keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
//ds[8]: [43] treeindex : [table = Channel, keys = (-channel:name), cond = (channel:name == Param (name)), value = ptr to 16]
//
extern TreeIndex<oid_t, size_t, 5000> idptr_ds_1;
...
public:
  ...
  oid_t activity_user_id;
    ...
    public:
      ...
      UserInActivity29(const kandan_small::PUser& p): user_id(p.id()),user_username(p.username()) {}
  ...
  Activity1(const kandan_small::PActivity& p): activity_id(p.id()),activity_created_at(p.created_at()),activity_updated_at(p.updated_at()),activity_action(p.action()),activity_content(p.content()),activity_channel_id(p.channel_id()),activity_user_id(p.user_id()) {}
...
extern SmallBasicArray<Activity1, 5000> activity_1;
...
extern SmallBasicArray<User4, 100> user_4;
...
public:
  ...
  Attachment10(): attachment_id(0),attachment_user_id(0) {}
  Attachment10(oid_t v0,oid_t v1): attachment_id(v0),attachment_user_id(v1) {}
  Attachment10(const kandan_small::PAttachment& p): attachment_id(p.id()),attachment_user_id(p.user_id()) {}
  inline void clear() { attachment_id = 0; }
  inline bool operator==(const Attachment10& other) const { return attachment_id==other.attachment_id; }
  void print() {
    printf("[attachment:id=%u, user_id=%u]\n", attachment_id, attachment_user_id);
  }
};
extern SmallBasicArray<Attachment10, 20> attachment_10;
inline size_t insert_attachment_10_by_key(Attachment10& v) {
  size_t insertpos = attachment_10.insert(v);
}
struct Attachment12 {
public:
  oid_t attachment_id;
  ...
  Attachment12(const kandan_small::PAttachment& p): attachment_id(p.id()),attachment_file_file_name(p.file_file_name()),attachment_file_content_type(p.file_content_type()),attachment_file_file_size(p.file_file_size()),attachment_message_id(p.message_id()),attachment_file_updated_at(p.file_updated_at()),attachment_created_at(p.created_at()),attachment_updated_at(p.updated_at()),attachment_user_id(p.user_id()),attachment_channel_id(p.channel_id()) {}
  inline void clear() { attachment_id = 0; }
...
extern SortedArray<ds_12_key_type, Attachment12, 20> ds_12;
...
public:
  ...
  oid_t channel_user_id;
    struct  ds_28_key_type {
      oid_t activity_id;
      ds_28_key_type(oid_t activity_id_): activity_id(activity_id_) {}
      ds_28_key_type(): activity_id(0) {}
      inline bool operator==(const ds_28_key_type& other) const { return (activity_id == other.activity_id); }
      inline bool operator<(const ds_28_key_type& other) const { return false || (true && activity_id < other.activity_id); }
      inline size_t get_hash() const { return (std::hash<oid_t>()(activity_id) << 0); }
    };
    SortedArray<ds_28_key_type, ItemPointer, 10> ds_28;
    inline size_t insert_ds_28_by_key(ds_28_key_type& key, ItemPointer& v) {
      size_t insertpos = ds_28.insert_by_key(key, v);
    }


    SmallBasicArray<ItemPointer, 10> activities_30;
  ...
  Channel16(const kandan_small::PChannel& p): channel_id(p.id()),channel_name(p.name()),channel_created_at(p.created_at()),channel_updated_at(p.updated_at()),channel_user_id(p.user_id()) {}
...
public:
  oid_t channel_id;
    ...
    SortedArray<ds_31_key_type, ItemPointer, 10> ds_31;
    inline size_t insert_ds_31_by_key(ds_31_key_type& key, ItemPointer& v) {
      size_t insertpos = ds_31.insert_by_key(key, v);
    }


  Channel22(): channel_id(0) {}
  Channel22(oid_t v0): channel_id(v0) {}
  Channel22(const kandan_small::PChannel& p): channel_id(p.id()) {}
  inline void clear() { channel_id = 0; }
...
struct  ds_27_key_type {
  oid_t channel_id;
  ds_27_key_type(oid_t channel_id_): channel_id(channel_id_) {}
  ds_27_key_type(): channel_id(0) {}
  inline bool operator==(const ds_27_key_type& other) const { return (channel_id == other.channel_id); }
  inline bool operator<(const ds_27_key_type& other) const { return false || (true && channel_id < other.channel_id); }
  inline size_t get_hash() const { return (std::hash<oid_t>()(channel_id) << 0); }
};
extern TreeIndex<ds_27_key_type, ItemPointer, 500> ds_27;
inline size_t insert_ds_27_by_key(ds_27_key_type& key, ItemPointer& v) {
  size_t insertpos = ds_27.insert_by_key(key, v);
}
struct  ds_35_key_type {
  oid_t activity_id;
  ds_35_key_type(oid_t activity_id_): activity_id(activity_id_) {}
  ds_35_key_type(): activity_id(0) {}
  inline bool operator==(const ds_35_key_type& other) const { return (activity_id == other.activity_id); }
  inline bool operator<(const ds_35_key_type& other) const { return false || (true && activity_id < other.activity_id); }
  inline size_t get_hash() const { return (std::hash<oid_t>()(activity_id) << 0); }
};
extern TreeIndex<ds_35_key_type, ItemPointer, 5000> ds_35;
inline size_t insert_ds_35_by_key(ds_35_key_type& key, ItemPointer& v) {
  size_t insertpos = ds_35.insert_by_key(key, v);
}
...
inline void init_ds_1_from_sql(MYSQL* conn) {
  std::string query_str("select activity.id,activity.created_at,activity.updated_at,activity.action,activity.content,activity.channel_id,activity.user_id from activities as activity  ");
...
//ds 10: [10] Basic array: Attachment, value = memobj(Attachment-id,user_id)
inline void init_ds_10_from_sql(MYSQL* conn) {
  std::string query_str("select attachment.id,attachment.user_id from attachments as attachment  ");
  ...
  size_t insertpos = 0;
  while (row != NULL) {
    Attachment10 value(str_to_uint(row[0]),str_to_uint(row[1]));
    insert_attachment_10_by_key(value);
    row = mysql_fetch_row(result);
  }
  mysql_free_result(result);
}
...
//ds 22: [22] sorted-array : [table = Channel, keys = (-channel:id), cond = (channel:id == Param (channel_id)), value = memobj(Channel-id)]
inline void init_ds_22_from_sql(MYSQL* conn) {
  oid_t key_0 = 0;
  std::string query_str("select channel.id as channel_id from channels as channel   group by channel_id order by channel_id");
  ...
  size_t insertpos = 0;
  while (row != NULL) {
    Channel22 value(str_to_uint(row[0]));
...
//ds 27: [27] treeindex : [table = Channel, keys = (-channel:id), cond = (channel:id == Param (channel_id)), value = ptr to 16]
inline void init_ds_27_from_sql(MYSQL* conn) {
  oid_t key_0 = 0;
  std::string query_str("select channel.id as channel_id from channels as channel   group by channel_id order by channel_id");
  ...
  size_t insertpos = 0;
  while (row != NULL) {
    size_t* pos = idptr_ds_16.find_by_key(str_to_uint(row[0]));
    ItemPointer ipos(INVALID_POS);
    if (pos != nullptr) ipos.pos = *pos;
    key_0 = str_to_uint(row[0]);
    ds_27_key_type key(key_0);
    if (pos != nullptr) insert_ds_27_by_key(key,ipos);
    row = mysql_fetch_row(result);
  }
  mysql_free_result(result);
}
//ds 35: [35] treeindex : [table = Activity, keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
inline void init_ds_35_from_sql(MYSQL* conn) {
  oid_t key_0 = 0;
  std::string query_str("select activity.id as activity_id from activities as activity   where activity.id >= 1 and activity.id <= 5000 group by activity_id order by activity_id");
  ...
  while (row != NULL) {
    ...
    if (pos != nullptr) ipos.pos = *pos;
    key_0 = str_to_uint(row[0]);
    ds_35_key_type key(key_0);
    if (pos != nullptr) insert_ds_35_by_key(key,ipos);
    row = mysql_fetch_row(result);
  }
  mysql_free_result(result);
}
...
//ds 28: [28] sorted-array : [table = Channel::ActivitiesInChannel(16), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
inline void init_ds_28_from_sql(MYSQL* conn, Channel16* upper_obj) {
  oid_t key_0 = 0;
  char qs[2000];
  sprintf(qs, "select activity.id as activity_id from activities as activity INNER JOIN channels as activity_channel ON activity.channel_id = activity_channel.id   where activity_channel.id = %u and activity.id >= 1 and activity.id <= 5000 group by activity_id order by activity_id", upper_obj->channel_id);
  std::string query_str(qs);
  ...
  while (row != NULL) {
    ...
    if (pos != nullptr) ipos.pos = *pos;
    key_0 = str_to_uint(row[0]);
    Channel16::ds_28_key_type key(key_0);
    if (pos != nullptr) upper_obj->insert_ds_28_by_key(key,ipos);
    row = mysql_fetch_row(result);
  }
  mysql_free_result(result);
}
...
//ds 31: [31] sorted-array : [table = Channel::ActivitiesInChannel(22), keys = (-activity:id), cond = ((activity:id >= Value (1)) && (activity:id <= Value (5000))), value = ptr to 1]
