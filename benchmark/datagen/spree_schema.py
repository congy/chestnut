...
product = Table('spree_prodcuts',scale)
variant = Table('spree_variants',scale*20)
price = Table('spree_prices', scale*40)
...
amount.set_value_generator(lambda: None if random.randint(0, 10)>6 else 100*random.random())
created_at = Field('created_at','date')
updated_at = Field('updated_at','date')
currency = Field('currency', 'string')
currency.value_with_prob = [('USD',50), ('CNY',10), ('EUR',10),('CAD',10),('AUD',10),('GBP',10)]

price.add_fields([amount, currency, created_at, updated_at])
